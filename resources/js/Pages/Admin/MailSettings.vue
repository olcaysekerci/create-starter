    ...
    <AdminLayout>
        <template #header>
            <PageHeader title="Mail Ayarları" description="E-posta gönderim ayarlarını yapılandırın">
                <template #actions>
                    <Button variant="secondary" @click="testMailModal = true">
            ...
            <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <span class="ml-2 text-gray-600 dark:text-gray-400">{{ mailSettings.host }}</span>
                    </div>
                    <div>
                        <span class="font-medium text-gray-700 dark:text-gray-300">Port:</span>
                        <span class="ml-2 text-gray-600 dark:text-gray-400">{{ mailSettings.port }}</span>
                    </div>
                    <div>
                        <span class="font-medium text-gray-700 dark:text-gray-300">Driver:</span>
                        <span class="ml-2 text-gray-600 dark:text-gray-400">{{ mailSettings.driver }}</span>
                    </div>
                    <div>
                        <span class="font-medium text-gray-700 dark:text-gray-300">Encryption:</span>
                        <span class="ml-2 text-gray-600 dark:text-gray-400">{{ mailSettings.encryption || 'Yok' }}</span>
                    </div>
                    <div>
                        <span class="font-medium text-gray-700 dark:text-gray-300">Gönderici:</span>
                        <span class="ml-2 text-gray-600 dark:text-gray-400">{{ mailSettings.from_address }}</span>
                    </div>
                    <div>
                        <span class="font-medium text-gray-700 dark:text-gray-300">Gönderici Adı:</span>
                        <span class="ml-2 text-gray-600 dark:text-gray-400">{{ mailSettings.from_name }}</span>
                    </div>
                </div>
            </div>
        ...
        <Modal :show="testMailModal" @close="testMailModal = false">
            <div class="p-6">
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <Button variant="secondary" @click="testMailModal = false">
                        İptal
                    </Button>
                    <Button variant="primary" @click="sendTestMail" :loading="sendingTest">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                        </svg>
                        Gönder
