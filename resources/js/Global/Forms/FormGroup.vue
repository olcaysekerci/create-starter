<template>
    <div :class="['form-group', { 'mb-6': !noMargin, 'mb-4': compact }]">
        ...
        <p v-if="helpText && helpPosition === 'before'" class="text-sm text-gray-600 mb-2">
            {{ helpText }}
        </p>
        ...
        <p v-if="helpText && helpPosition === 'after'" class="mt-2 text-sm text-gray-600">
            {{ helpText }}
        </p>
        
        <!-- Success message -->
        <p v-if="success" class="mt-2 text-sm text-green-600 flex items-center">
            <svg class="w-4 h-4 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
